...
if ( ! class_exists( 'Wolf_Tour_Dates' ) ) {
	...
	class Wolf_Tour_Dates {
		...
		public function register_taxonomy() {
			...
			$labels = array(
				'name' => __( 'Artists', 'wolf' ),
				'singular_name' => __( 'Artist', 'wolf' ),
				'search_items' => __( 'Search Artists', 'wolf' ),
				'popular_items' => __( 'Popular Artists', 'wolf' ),
				'all_items' => __( 'All Artists', 'wolf' ),
				'parent_item' => __( 'Parent Artist', 'wolf' ),
				'parent_item_colon' => __( 'Parent Artist:', 'wolf' ),
				'edit_item' => __( 'Edit Artist', 'wolf' ),
				'update_item' => __( 'Update Artist', 'wolf' ),
				'add_new_item' => __( 'Add New Artist', 'wolf' ),
				'new_item_name' => __( 'New Artist', 'wolf' ),
				'separate_items_with_commas' => __( 'Separate artists with commas', 'wolf' ),
				'add_or_remove_items' => __( 'Add or remove artists', 'wolf' ),
				'choose_from_most_used' => __( 'Choose from the most used artists', 'wolf' ),
				'menu_name' => __( 'Artists', 'wolf' ),
			);
			...
			register_taxonomy( 'artist', array( 'show' ), $args );
		}
		...
		public function add_options_menu() {
			...
			$post_type_name = 'show';
			add_submenu_page( 'edit.php?post_type='.$post_type_name, __( 'Settings', 'wolf' ), __( 'Settings', 'wolf' ), 'edit_plugins', 'wolf-tour-dates-options', array( $this, 'options_form' ) );
		}
		 ...
		 * @return string
		 */
		public function get_option( $value = null, $default = null ) {
		...
		public function register_settings() {
			...
			add_settings_field( 'upcoming_shows_text', __( 'Upcoming Shows Text', 'wolf' ), array( $this, 'setting_upcoming_shows_text' ), 'wolf-tour-dates-settings', 'wolf-tour-dates-settings' );
			add_settings_field( 'past_shows_text', __( 'Past Shows Text', 'wolf' ), array( $this, 'setting_past_shows_text' ), 'wolf-tour-dates-settings', 'wolf-tour-dates-settings' );
			add_settings_field( 'no_shows_text', __( 'No Shows Text', 'wolf' ), array( $this, 'setting_no_shows_text' ), 'wolf-tour-dates-settings', 'wolf-tour-dates-settings' );
		 ...
		 * Upcoming Shows Text
		 */
		public function setting_upcoming_shows_text() {
			?>
			<input type="text" value="<?php echo $this->get_option( 'upcoming_shows_text', __( 'Upcoming Shows', 'wolf' ) ); ?>" name="wolf_tour_dates_settings[upcoming_shows_text]">
			<?php
		}
		 ...
		 * Past Shows Text
		 */
		public function setting_past_shows_text() {
			?>
			<input type="text" value="<?php echo $this->get_option( 'past_shows_text', __( 'Past Shows', 'wolf' ) ); ?>" name="wolf_tour_dates_settings[past_shows_text]">
			<?php
		}
		 ...
		 * Past Shows Text
		 */
		public function setting_no_shows_text() {
			?>
			<input type="text" value="<?php echo $this->get_option( 'no_shows_text', __( 'No upcoming shows scheduled', 'wolf' ) ); ?>" name="wolf_tour_dates_settings[no_shows_text]">
			<?php
		}
		 ...
		 * @return object
		 */
		public function future_shows_query( $count, $artist ) {
		 ...
		 * @return object
		 */
		public function past_shows_query( $count, $artist ) {
			add_filter( 'posts_orderby', array( $this, 'order_by' ), 10, 1 );
		 ...
		 * @return string
		 */
		public function tour_dates( $count = 100, $past = null, $link = null, $artist = null ) {
			...
			$upcoming_shows = $this->future_shows_query( $count, $artist );
			$past_shows     = $this->past_shows_query( $count, $artist );
			...
			if ( $upcoming_shows->have_posts() ) :
				...
				if ( $display_past_show ) {
					...
					echo '<h2>';
					echo $this->get_option( 'upcoming_shows_text', __( 'Upcoming Shows', 'wolf' ) );
					echo '</h2>';
				...
				while ( $upcoming_shows->have_posts() ) : $upcoming_shows->the_post();
					...
					<tr class="wolf-single-date <?php echo sanitize_html_class( $custom_class );  ?>">
						...
						?><span title="<?php _e( 'View flyer', 'wolf' ); ?>"><a class="wolf-show-flyer" href="<?php echo esc_url( $img ); ?>"></a></span>
						<?php endif; ?>
						</td>
				...
				?><p><?php echo $this->get_option( 'no_shows_text', __( 'No upcoming shows scheduled', 'wolf' ) ); ?></p><?php
			...
			if ( $past_shows->have_posts() && $display_past_show ) :
				...
				echo '<h2>';
				echo $this->get_option( 'past_shows_text', __( 'Past Shows', 'wolf' ) );
				echo '</h2>';
		 ...
		 * @return string
		 */
		public function widget( $count, $url, $link_text, $artist ) {
			ob_start();
			$wpdb = $this->wpdb;
			...
			if ( $upcoming_shows->have_posts() ) {
				echo '<table class="wolf-upcoming-shows wolf-upcoming-shows-widget-table wolf-shows">';
				while ( $upcoming_shows->have_posts() ) : $upcoming_shows->the_post();
				...
				if ( $url && $link_text ) {
					echo '<p><a href="' . esc_url( $url ) . '" class="wolf-more-dates">' . $link_text . '</a></p>';
				}
			} else {
		...
		public function shortcode( $atts ) {
			...
			ob_start();
			$this->tour_dates( $count, $past, $link, $artist );
			return ob_get_clean();
		}
